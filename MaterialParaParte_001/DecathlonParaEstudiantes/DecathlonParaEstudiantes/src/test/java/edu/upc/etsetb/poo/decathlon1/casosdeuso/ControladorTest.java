...
//            System.out.println(ex.getStackTrace()[0].toString() + "\n" + ex.getMessage());
    ...
    public void test05_anyadirMarcaEnEventoDeUnAtleta() {
        ...
        try {
            ...
            size = atletas.size();
            resultado = controlador.anyadirMarcaEnEventoDeUnAtleta(1, 1, 12.0);
            esperado = Controlador.NO_ATLETAS_INSCRITOS;
            AssertionError error1 = this.sAssertEquals(esperado, resultado, valorTotal * 0.05, "El "
                    + "método debería haber devuelto el código Controlador.NO_ATLETAS"
                    + "_INSCRITOS (1) porque no hay ningún atleta inscrito. En "
                    + "su lugar ha devuelto " + resultado);
            toThrow = toThrow(error1, toThrow);
            error = this.sAssertEquals(0, atletas.size(), valorTotal * 0.05, "El "
        ...
        try {
            ...
            resultado = controlador.anyadirMarcaEnEventoDeUnAtleta(-1, 1, 12.0);
            esperado = Controlador.NUM_INSCRIPCION_ERRONEO;
            size = atletas.size();
            error = this.sAssertEquals(esperado, resultado, valorTotal * 0.05, "El "
                    + "método debería haber devuelto el código Controlador. "
                    + "NUM_INSCRIPCION_ERRONEO (2) porque se ha pasado como número "
                    + "de inscripción el valor -1. En su lugar ha devuelto " + resultado);
            toThrow = toThrow(error, toThrow);
            error = this.sAssertEquals(1, atletas.size(), valorTotal * 0.05, "El "
                    ...
                    + size + " atletas.");
            toThrow = toThrow(error, toThrow);
            this.checkSiSeHaAnyadidoMarcaAAtleta(atletas, 1, valorTotal * 0.05);
        } catch (Exception ex) {
            this.printlnAlways("*** Se ha capturado una excepción que probablemente "
                    + "ha sido lanzada por tu código. Mira la traza para "
                    + "detectar en qué punto ha sido creada y lanzada...");
        ...
        try {
            ...
            atletas = (Map<Integer, Atleta>) attr.get();
            sizeBefore = atletas.size();
            resultado = controlador.anyadirMarcaEnEventoDeUnAtleta(7, 1, 12.0);
            size = atletas.size();
            esperado = Controlador.NUM_INSCRIPCION_ERRONEO;
            error = this.sAssertEquals(esperado, resultado, valorTotal * 0.05, "El "
                    + "método debería haber devuelto el código Controlador. "
                    + "NUM_INSCRIPCION_ERRONEO (2) porque se ha pasado como número "
                    + "de inscripción un valor mayor que el de número de atletas "
                    + "inscritos. En su lugar ha devuelto " + resultado);
            toThrow = toThrow(error, toThrow);
            error = this.sAssertEquals(1, atletas.size(), valorTotal * 0.05, "El "
                    + "método no debería haber añadido ningún atleta al mapa "
                    + "de atletas. Antes tenía " + sizeBefore + ". Y ahora tiene "
        ...
        try {
            ...
            resultado = controlador.anyadirMarcaEnEventoDeUnAtleta(1, -1, 12.0);
            size = atletas.size();
            esperado = Controlador.TIPO_DE_EVENTO_ERRONEO;
            error = this.sAssertEquals(esperado, resultado, valorTotal * 0.05, "El "
                    + "método debería haber devuelto el código Controlador. "
                    + "TIPO_DE_EVENTO_ERRONEO (3) porque se ha pasado como identificador "
                    + "del evento el valor -1. En su lugar ha devuelto " + resultado);
            toThrow = toThrow(error, toThrow);
            error = this.sAssertEquals(1, atletas.size(), valorTotal * 0.05, "El "
                    + "método no debería haber añadido ningún atleta al mapa "
                    + "de atletas. Antes tenía " + sizeBefore + ". Y ahora tiene "
            ...
            sizeBefore = atletas.size();
            resultado = controlador.anyadirMarcaEnEventoDeUnAtleta(1, 20, 12.0);
            size = atletas.size();
            esperado = Controlador.TIPO_DE_EVENTO_ERRONEO;
            error = this.sAssertEquals(esperado, resultado, valorTotal * 0.05, "El "
                    + "método debería haber devuelto el código Controlador. "
                    + "TIPO_DE_EVENTO_ERRONEO (3) porque se ha pasado como identificador "
                    + "del evento un valor mayor que el número de eventos "
                    + "registrados MarcaEnEvento.NUM_EVENTOS) (5). En su lugar "
                    + "ha devuelto " + resultado);
            toThrow = toThrow(error, toThrow);
            error = this.sAssertEquals(1, atletas.size(), valorTotal * 0.05, "El "
                    + "método no debería haber añadido ningún atleta al mapa "
                    + "de atletas. Antes tenía " + sizeBefore + ". Y ahora tiene "
        ...
        try {
            this.testsAnyadirMarcaCaso4(valorTotal * 0.3);
        } catch (Exception ex) {
            this.printlnAlways("*** Se ha capturado una excepción que probablemente "
                    + "ha sido lanzada por tu código. Mira la traza para "
                    + "detectar en qué punto ha sido creada y lanzada...");
    ...
    public void test_getClasificacionNoAtletasInscritos(double valor) {
        ...
        try {
            ...
            error = this.sAssertEquals(InterficieUsuario.NO_ATLETAS_INSCRITOS_STR,
                    result, valor, "Al no haber atletas inscritos, el método "
                    + "debería devolver \'" + InterficieUsuario.NO_ATLETAS_INSCRITOS_STR
                    + "\'. En su lugar ha devuelto \'" + result + "\'");
            toThrow = toThrow(error, toThrow);
        } catch (Exception ex) {
            this.printlnAlways("*** Se ha capturado una excepción que probablemente "
                    + "ha sido lanzada por tu código. Mira la traza para "
    ...
    public void test_getClasificacionMalNumeroAtletas(double valor) {
        ...
        try {
            ...
            String result = controlador.getClasificacion(0);
            error = this.sAssertEquals(InterficieUsuario.NUM_ATLETAS_ERRONEO_STR,
                    result, valor * 0.333333333, "Al pasar el argumento 0, el método "
                    + "debería devolver \'" + InterficieUsuario.NUM_ATLETAS_ERRONEO_STR
                    + "\'. En su lugar ha devuelto \'" + result + "\'");
            toThrow = toThrow(error, toThrow);
            //
            ...
            error = this.sAssertEquals(InterficieUsuario.NUM_ATLETAS_ERRONEO_STR,
                    result, valor * 0.333333333, "Al pasar un argumento negativo, el método "
                    + "debería devolver \'" + InterficieUsuario.NUM_ATLETAS_ERRONEO_STR
                    + "\'. En su lugar ha devuelto \'" + result + "\'");
            toThrow = toThrow(error, toThrow);
            //
            ...
            error = this.sAssertEquals(InterficieUsuario.NUM_ATLETAS_ERRONEO_STR,
                    result, valor * 0.333333333, "Al pasar un argumento mayor que "
                    + "el número de atletas inscritos, el método "
                    + "debería devolver \'" + InterficieUsuario.NUM_ATLETAS_ERRONEO_STR
                    + "\'. En su lugar ha devuelto \'" + result + "\'");
            toThrow = toThrow(error, toThrow);
        } catch (Exception ex) {
    ...
    public void test_getClasificacionNumeroAtletasOK(double valor) {
        ...
        try {
            ...
            String esperadoParaComparar = "1alglarbibourrada2kazdmitriykarpov3frakevinmayer4"
    ...
    private void testsAnyadirMarcaCaso4(double valor) {
        AssertionError error = null;
        AssertionError toThrow = null;
        try {
            this.printlnAlways("\tCaso 4: se añade una marca de un atleta en un evento correctos");
            Optional<Object> attr = this.getPrivateFieldValue(controlador, "atletas");
            if (!attr.isPresent()) {
                error = this.sAssertTrue(false, 0, "No parece haberse creado ningún contenedor "
                        + "para el atributo \'atletas\'");
            ...
            int resultado = controlador.anyadirMarcaEnEventoDeUnAtleta(1, 2, 12.0);
            int size = atletas.size();
            int esperado = Controlador.RESULTADO_OK;
            this.printlnAlways("\t  Prueba 1: el método devuelve Controlador.RESULTADO_OK (0)");
            error = this.sAssertEquals(esperado, resultado, valor * 0.16666666, "El "
                    + "método debería haber devuelto el código Controlador. "
                    + "RESULTADO_OK (0) porque los argumentos pasados son "
                    + "correctos. En su lugar ha devuelto " + resultado);
            toThrow = toThrow(error, toThrow);
            ...
            if (error == null) {
                this.printlnAlways("\t  Prueba 3: el evento del objeto MarcaEnEvento es correcta");
                esperado = 2;
                ...
                if (!optEv.isPresent()) {
                    error = this.sAssertTrue(false, 0, "No parece que el objeto MarcaEnEvento "
                            + "tenga el atributo \'evento\'.");
                    toThrow = toThrow(error, toThrow);
                } else {
                    error = this.sAssertEquals(esperado, (int) optEv.get(), valor * 0.16666666,
                            "El evento en el objeto MarcaEnEvento no es correcto. "
                            + "Su valor es " + (int) optEv.get() + ". Sin embargo, "
